...
void setup() {
  ...
  //switchChar.setEventHandler(BLEWritten, receiveHandler);
  // set an initial value for the characteristic
  switchChar.setValue(0, 20);
...
void switchCharacteristicWritten(BLEDevice central, BLECharacteristic characteristic) {
  ...
  //  const byte midiData[20] ={0,};
  //  if (switchChar.value()) {
  //    const unsigned char *data = switchChar.value();
  //    //Serial.println(switchChar.value(),DEC);
  //    Serial.println(switchChar.valueLength());
...
void receiveHandler(BLEDevice central, BLECharacteristic characteristic) {
  size_t len = switchChar.valueLength();
  const unsigned char *data = switchChar.value();
  addReceiveBytes((const uint8_t*)data, len);
